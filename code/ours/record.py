...
class SelectionRecord(Record):
    ...
    def get_redundancy(self,num=25, rm = None):
        ordered, performance = self._get_ordered()
        ordered = ordered.numpy()
        redundancy = 0.0
        for i in range(0,ordered.shape[0]):
            j = i+1
            while j<ordered.shape[0]:
                    redundancy = redundancy+rm[ordered[i]][ordered[j]]
                    j=j+1
        return torch.tensor(redundancy).type(torch.FloatTensor).repeat(num+1, 1)
